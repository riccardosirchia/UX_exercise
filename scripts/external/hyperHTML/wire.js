...
const content = type => {
  ...
  return function () {
    ...
    if (template !== args[0]) {
      //console.log('content: created new Tagger', args[0], template);
      template = args[0];
      tagger = new Tagger(type);
      wire = wireContent(tagger.apply(tagger, args));
    } else {
      //console.log('reusing old tagger',  args[0], template);
...
const weakly = (obj, type) => {
  ...
  if (!wire) {
	wires.set(obj, wire = {});
// 	console.log('created new wire for', obj, id);
//   }	else {
// 	console.warn('using old wire for', obj, id);
  }
