        ...
        render() {
            const selectOptions = this.config.options.map((option) => {
                ...
                return hyperHTML.wire()`
                    <option id=${option.value} selected=${selected} value=${option.value} onclick=${option.action || this.selectOption.bind(this)}>
                        ${option.text}
                    </option>`
            })
            return hyperHTML.wire()`
                <custom-select onclick=${this.toggleDropdown.bind(this)} id = ${this.config.id}>
                    <custom-select-input class="">
                        <ui-label id="custom-select-label">${this.selectedOption.value || 'Please select domain...'}</ui-label>
